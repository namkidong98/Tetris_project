...
void InitTetris(){
	...
	//
	DrawBlock(recommendY, recommendX, nextBlock[0], recommendR, 'R'); //DrawRecommend
	//
...
void recommendedPlay() {
	int command, i, j; int tmp_blockY; time_t start, stop; double playtime;
	clear();
	act.sa_handler = BlockDown;
	sigaction(SIGALRM, &act, &oact);
	...
	do {
		...
		if (blockRotate != recommendR || blockX != recommendX) { 
			//이전 위치 지우기
			tmp_blockY = blockY;
			while (CheckToMove(field, nextBlock[0], blockRotate, tmp_blockY + 1, blockX)) tmp_blockY++;
			for (i = 0; i < 4; i++) {
				for (j = 0; j < 4; j++) {
	...
	stop = time(NULL);
	playtime = (double)difftime(stop, start);
	...
	clear();
	move(0, 0);
	printw("Play time(= t): %lf seconds\n", playtime);
	printw("score(t): %d\n", score);
	printw("time(t): %lf\n", t_time);
	printw("space(t): %ld\n", t_space);
	printw("Time Efficiency: %lf\n", (double)score / t_time);
	printw("Space Efficiency: %ld\n", (long)score / t_space);
	t_time = 0;
	getch();
}
...
int recommend(RecNode root) {
	...
	for (rotation = 0; rotation < 4; rotation++) { //each rotation of block
		blockX = WIDTH / 2 - 2;
		blockY = -1;
		while (CheckToMove(root.recField, nextBlock[root.level], rotation, blockY, blockX - 1)) {
			blockX--;
		} //멈추는 지점의 blockX가 제일 좌측, 즉 시작지점이다

		for (term = 0; term <= 9; term++) {
			tmp_blockX = blockX + term;
			while (CheckToMove(root.recField, nextBlock[root.level], rotation, blockY + 1, tmp_blockX)) {
				blockY++;
			} //멈추는 지점이 block이 필드와 만나는 지점의 blockY와 blockX이다

			//root_pointer->child[count] = temp[count];
			...
			result += AddBlockToField(temp[count].recField, nextBlock[root.level], temp[count].recBlockRotate, temp[count].recBlockY, temp[count].recBlockX);
			...
			if (temp[count].accumulatedScore >= max && temp[count].level == 1) { //현재까지 최대 누적 점수와 비교해서, 가장 큰 누적 점수를 갖는 블록의 회전수와 위치를 갱신
			...
			else if (temp[count].accumulatedScore >= max) max = temp[count].accumulatedScore;

			blockY = -1;
			result = root.accumulatedScore;
			if (CheckToMove(root.recField, nextBlock[root.level], rotation, -1, tmp_blockX + 1));
			else break;
...
int modified_recommend(RecNode root) {
	...
	RecNode temp[40];
	int max_rotation = 4;
	if (nextBlock[root.level] == 4) max_rotation = 1;
	if (nextBlock[root.level] == 0 || nextBlock[root.level] == 5 || nextBlock[root.level] == 6) max_rotation = 2;
	for (rotation = 0; rotation < max_rotation; rotation++) { //each rotation of block
		blockX = WIDTH / 2 - 2;
		blockY = -1;
		while (CheckToMove(root.recField, nextBlock[root.level], rotation, blockY, blockX - 1)) {
			blockX--;
		...
		for (term = 0; term <= 9; term++) {
			...
			result += (AddBlockToField(temp[count].recField, nextBlock[root.level], temp[count].recBlockRotate, temp[count].recBlockY, temp[count].recBlockX) * 2);
			result += (DeleteLine(temp[count].recField) * 15);
			result += (touch_plus * 5);
			touch_plus = 0;

			if (temp[count].level < MAX_LEVEL) {
				temp[count].accumulatedScore = result;
				temp[count].accumulatedScore = modified_recommend(temp[count]);
			}
			else {
				temp[count].accumulatedScore = result;
			}
...
void BlockDown(int sig) {
	...
	if (CheckToMove(field, nextBlock[0], blockRotate, blockY + 1, blockX)) {
		...
		if (blockY == -1) gameOver = 1;
			...
			modified_recommend(root);
			stop = time(NULL);
			t_time += (double)difftime(stop, start);  //t_time는 전역 변수로 설정(누적 시간이니 += 사용)
...
void DrawChange(char f[HEIGHT][WIDTH],int command,int currentBlock,int blockRotate, int blockY, int blockX){
	...
	case KEY_UP:
		...
		while (CheckToMove(f, currentBlock, pre_Rotate, tmp_blockY + 1, blockX)) tmp_blockY++;
		for (i = 0; i < 4; i++) {
			for (j = 0; j < 4; j++) {
	...
	case KEY_RIGHT:
		pre_blockX = blockX - 1;
		while (CheckToMove(f, currentBlock, blockRotate, tmp_blockY + 1, pre_blockX)) tmp_blockY++;
		for (i = 0; i < 4; i++) {
			for (j = 0; j < 4; j++) {
				if (block[currentBlock][blockRotate][i][j] == 1) {
...
int AddBlockToField(char f[HEIGHT][WIDTH],int currentBlock,int blockRotate, int blockY, int blockX){
	int i, j; int touched = 0; int flag = 0;
	for (i = 0; i < 4; i++) {
		flag = 0;
		for (j = 0; j < 4; j++) {
			if (block[currentBlock][blockRotate][i][j] == 1) {
				...
				if (f[blockY + i + 1][blockX + j] == 1) touched++; //한칸 아래 필드와 맞닿는 경우
				else if ((blockY + i) == (HEIGHT - 1)) touched++;  //밑바닥과 맞닿는 경우

				if (f[blockY + i][blockX + j + 1] == 1) touch_plus++;
				else if ((blockX + j + 1) == (WIDTH)) touch_plus++;

				if (flag == 1 && f[blockY + i][blockX + j - 1] == 1) touch_plus++;
				else if (flag == 1 && (blockX + j - 1) == -1) touch_plus++;
			}
		}
	}
	return touched * 10;
...
void DrawShadow(int y, int x, int blockID,int blockRotate){
	int tmp_blockY = y;
	while (CheckToMove(field, blockID, blockRotate, tmp_blockY + 1, x)) {
		tmp_blockY++;
	}
	DrawBlock(tmp_blockY, x, blockID, blockRotate, '/');
}
